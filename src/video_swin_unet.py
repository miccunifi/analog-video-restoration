...
class BasicLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        for blk in self.blocks:
            x = blk(x, attn_mask)
        # x = x.view(B, D, H, W, -1)
        x = rearrange(x, 'b d h w c -> b d c h w').reshape(B*D, C, H, W)
        x = self.last_conv(x)
        x = rearrange(x.reshape(B, D, C, H, W), 'b d c h w -> b d h w c')
        x = x + residual
...
class BasicUpLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if upsample is not None:
            self.upsample = upsample(embed_dim=dim)
        else:
            self.upsample = None
...
class VideoSwinEncoderDecoder(nn.Module):
    def __init__(self,
                 in_chans=3,
                 embed_dim=64,
                 depths=[2, 2, 6, 2],
        ...
        super(VideoSwinEncoderDecoder, self).__init__()
        self.embed_dim = embed_dim

        self.conv_input = nn.Conv3d(in_chans, embed_dim, kernel_size=3, stride=(1, 2, 2), padding=1)
        ...
        self.decoding_layers.append(PatchExpand(embed_dim * 2 ** (self.num_layers - 1)))
        ...
        self.conv_output = nn.Conv2d(embed_dim // 2, in_chans, 3, 1, 1)
    ...
    def forward_decoding(self, x, residual):
        for i, layer in enumerate(self.decoding_layers):
            x = layer(x + residual[-1 - i])
        x = rearrange(x, 'b c d h w -> b d c h w')
        B, T, C, H, W = x.shape
        x = self.conv_output(x.reshape(B * T, C, H, W))
        x = x.reshape(B, T, -1, H, W)
        return x

    def forward(self, x):
        out = x.clone()
        x, residual = self.forward_encoding(x)
        x = self.forward_decoding(x, residual)
        return out + x
